...
- The X-ray homolog build additionally writes `*_rejected.csv` beside each
  95%/100% homolog CSV. Only entries whose sole experimental method is
  `X-RAY DIFFRACTION` are considered. The reports contain one row for every
  such RCSB sequence hit rejected by the modeled-core coordinate filter,
  including its source NMR entry, X-ray polymer entity, chains, cutoff, and
  reason.
